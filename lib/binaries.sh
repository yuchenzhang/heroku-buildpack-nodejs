...
install_nodejs() {
  ...
  # if [ "$code" != "200" ]; then
    ...
    if [ -f "/tmp/node-v$number-$platform.tar.gz" ]; then
      echo "Copying predownloaded /tmp/node-v$number-$platform.tar.gz"
      cp /tmp/node-v$number-$platform.tar.gz /tmp/node.tar.gz
    else
      false
    fi
  # fi
