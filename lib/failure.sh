...
failure_message() {
  ...
  echo ""
  echo "Stuck? Visit https://help.heroku.com for help."
  echo " - Heroku"
  echo ""
}
...
warn_node_engine() {
  local node_engine=${1:-}
  if [ "$node_engine" == "" ]; then
    save_warning "Node version not specified in package.json" "https://devcenter.heroku.com/articles/nodejs-support#specifying-a-node-js-version"
    mcount 'warnings.node.unspecified'
  elif [ "$node_engine" == "*" ]; then
    save_warning "Dangerous semver range (*) in engines.node" "https://devcenter.heroku.com/articles/nodejs-support#specifying-a-node-js-version"
...
warn_prebuilt_modules() {
  ...
  if [ -e "$build_dir/node_modules" ]; then
    save_warning "node_modules checked into source control" "https://blog.heroku.com/node-habits-2016#9-only-git-the-important-bits"
...
warn_missing_package_json() {
  ...
  if ! [ -e "$build_dir/package.json" ]; then
    save_warning "No package.json found"
...
warn_old_npm() {
  ...
  if [ "${npm_version:0:1}" -lt "2" ]; then
    ...
    save_warning "This version of npm ($npm_version) has several known issues - consider upgrading to the latest release ($latest_npm)" "https://devcenter.heroku.com/articles/nodejs-support#specifying-an-npm-version"
...
warn_young_yarn() {
  if $YARN; then
    save_warning "This project was built with yarn, which is new and under development. Some projects can still be built more reliably with npm" "https://devcenter.heroku.com/articles/nodejs-support#build-behavior"
...
warn_untracked_dependencies() {
  ...
  if grep -qi 'gulp: not found' "$log_file" || grep -qi 'gulp: command not found' "$log_file"; then
    save_warning "Gulp may not be tracked in package.json" "https://devcenter.heroku.com/articles/troubleshooting-node-deploys#ensure-you-aren-t-relying-on-untracked-dependencies"
    mcount 'warnings.modules.untracked.gulp'
  fi
  if grep -qi 'grunt: not found' "$log_file" || grep -qi 'grunt: command not found' "$log_file"; then
    save_warning "Grunt may not be tracked in package.json" "https://devcenter.heroku.com/articles/troubleshooting-node-deploys#ensure-you-aren-t-relying-on-untracked-dependencies"
    mcount 'warnings.modules.untracked.grunt'
  fi
  if grep -qi 'bower: not found' "$log_file" || grep -qi 'bower: command not found' "$log_file"; then
    save_warning "Bower may not be tracked in package.json" "https://devcenter.heroku.com/articles/troubleshooting-node-deploys#ensure-you-aren-t-relying-on-untracked-dependencies"
...
warn_angular_resolution() {
  ...
  if grep -qi 'Unable to find suitable version for angular' "$log_file"; then
    save_warning "Bower may need a resolution hint for angular" "https://github.com/bower/bower/issues/1746"
...
warn_missing_devdeps() {
  ...
  if grep -qi 'cannot find module' "$log_file"; then
    save_warning "A module may be missing from 'dependencies' in package.json" "https://devcenter.heroku.com/articles/troubleshooting-node-deploys#ensure-you-aren-t-relying-on-untracked-dependencies"
    ...
    if [ "$NPM_CONFIG_PRODUCTION" == "true" ]; then
      ...
      if [ "$devDeps" != "" ]; then
        save_warning "This module may be specified in 'devDependencies' instead of 'dependencies'" "https://devcenter.heroku.com/articles/nodejs-support#devdependencies"
...
warn_econnreset() {
  ...
  if grep -qi 'econnreset' "$log_file"; then
    save_warning "ECONNRESET issues may be related to npm versions" "https://github.com/npm/registry/issues/10#issuecomment-217141066"
